                            ...
                            <div class="sub_page">
                                ...
                                <table class="txt project_txt counseling">
                                    <tr>
                                        ...
                                        <th class="exam_name">제목</th>
                                        <th class="join_time">참여한날짜</th>
                                        <th class="apply">상세보기</th>
                                    </tr>
                                    
                                    <c:forEach var="item" items="${model.list}" varStatus="status">
                                    <tr>
                                        <td>${model.itemtotalcount - (status.index + model.beforeDomain.PAGE *  model.beforeDomain.ITEM_COUNT)}</td>
                                        <td class="align_center">${item.name }</td>
                                        <td>${fn:substring(item.complete_tm,0,11)}</td>
                                        <td class="align_center">
                                        	<a href="/user/exam/result/view.do?member_id=${item.member_id}&exam_idx=${item.exam_idx}&idx=${item.idx}">상세보기</a>
                                        </td>
                                    </tr>
                                    </c:forEach>
                                </table>
