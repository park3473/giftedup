                                            ...
                                            <div class="form_01">
                                                <div class="page_seach">
                                                    ...
                                                    <select ID="ADDRESS_LOCAL" NAME="ADDRESS_LOCAL">
                                                        ...
                                                        <option value="세종" <c:if test="${model.beforeDomain.ADDRESS_LOCAL == '세종특별자치시'}">selected</c:if>>세종특별자치시</option>
                                                    ...
                                                    <select ID="FILE_TYPE" NAME="FILE_TYPE">
                                                        <option value="">서류 여부</option>
                                                        <option value="1" <c:if test="${model.beforeDomain.FILE_TYPE == '1'}">selected</c:if>>대기</option>
                                                        <option value="2" <c:if test="${model.beforeDomain.FILE_TYPE == '2'}">selected</c:if>>완료</option>
                                                    </select>
                                    ...
                                    <table id="gcTable" class="tablesorter">
                                        ...
                                        <tbody>
                                            ...
                                            <tr >
                                                ...
                                                <td onclick="update('${item.TYPE }','${item.NAME}','${item.BIRTH }','${item.LEVEL }')">${model.itemtotalcount - (var.index + model.beforeDomain.PAGE *  model.beforeDomain.ITEM_COUNT)}</td>
                                                <td onclick="update('${item.TYPE }','${item.NAME}','${item.BIRTH }','${item.LEVEL }')">
                                                ...
                                                <td onclick="update('${item.TYPE }','${item.NAME}','${item.BIRTH }','${item.LEVEL }')">${item.NAME}</td>
                                                <td onclick="update('${item.TYPE }','${item.NAME}','${item.BIRTH }','${item.LEVEL }')">${item.SEX}</td> 
                                                <td onclick="update('${item.TYPE }','${item.NAME}','${item.BIRTH }','${item.LEVEL }')">
                                                    <!--${item.ADDRESS_LOCAL}-->
                                                    ...
                                                    <c:if test="${item.ADDRESS_LOCAL == '세종특별자치시'}">세종특별자치시</c:if>
                                                ...
                                                <td onclick="update('${item.TYPE }','${item.NAME}','${item.BIRTH }','${item.LEVEL }')">${item.BIRTH}</td>
                                                <td onclick="update('${item.TYPE }','${item.NAME}','${item.BIRTH }','${item.LEVEL }')">${item.SCHOOL_NAME}</td>
                                                <td onclick="update('${item.TYPE }','${item.NAME}','${item.BIRTH }','${item.LEVEL }')">
                                                    <!-- 학생-->
                                                    ...
                                                    <!-- 교사 -->
                                                    <c:if test="${item.SCHOOL_TYPE  == '초등교사'}">초등교사</c:if>
                                                    <c:if test="${item.SCHOOL_TYPE  == '중등교사'}">중등교사</c:if>
                                                </td>
                                                <td onclick="update('${item.TYPE }','${item.NAME}','${item.BIRTH }','${item.LEVEL }')">
                                                    <!--${item.SCHOOL_YEAR}-->
                                                ...
                                                <td onclick="update('${item.TYPE }','${item.NAME}','${item.BIRTH }','${item.LEVEL }')">${item.PHONE}</td>
                                                <td onclick="update('${item.TYPE }','${item.NAME}','${item.BIRTH }','${item.LEVEL }')" id="ELIGIBILITY">${item.ELIGIBILITY}</td>
                                                <td onclick="update('${item.TYPE }','${item.NAME}','${item.BIRTH }','${item.LEVEL }')" class="td_08">${fn:substring(item.CREATE_TM,0,11)}</td>
                                                <td onclick="update('${item.TYPE }','${item.NAME}','${item.BIRTH }','${item.LEVEL }')" > 
...
<script>
    ...
    function searchBtnClick() {
        ...
        URL = URL + "&SCHOOL_YEAR=" +$('#SCHOOL_YEAR').val();
        URL = URL + "&SEARCH_TEXT=" + encodeURI($('#SEARCH_TEXT').val());
        URL = URL + "&FILE_TYPE=" + encodeURI($('#FILE_TYPE').val());
        URL = URL + "&SEARCH_TYPE=" + $('#SEARCH_TYPE').val();
        location.href = URL;
    }
