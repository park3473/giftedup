                    ...
                    <form action="./update.do" method="post" name="insertForm" id="insertForm">
                                ...
                                <div class="member_input_wrap">
                                    <ul class="member_input">
                                        <li>
                                            ...
                                            <input class="input_title" type="text" id="title" name="title" value="${model.view.title }">
                                        </li>
                                        <li>
                                            <span class="list_t">시작 일시</span>
                                            <input class="input_size datecalendar2" type="text" id="start" name="start" autocomplete="off" readonly="readonly" value="${fn:substring(model.view.start,0,11) }" >
                                        </li>
                                        <li>
                                            <span class="list_t">종료 일시</span>
                                            <input class="input_size datecalendar2" type="text" id="end" name="end" autocomplete="off" readonly="readonly" value="${fn:substring(model.view.end,0,11) }">
                                        </li>
                                        <c:forEach var="item" items="${model.require }" varStatus="status">
                                        	<li id="requirement" class="require" data-type="update">
                                        		<input type="hidden" name="require_idx" value="${item.idx }">
                                        		<span class="list_t">필수 정보</span><label>필수값 이름</label><input type="text" name="require_name" value="${item.require_name }">
                                        		<label>필수값 설명</label><input type="text" name="require_content" value="${item.require_content }">
                                        		<button type="button" onclick="removeRequirement2(this)">삭제</button>
                                        	</li>
                                        </c:forEach>
                                        <!-- 필수사항 들어가는 부분  -->
                                        ...
                                        <li>
                                        	...
                                        	<select name="file_cnt">
                                        		...
                                        		<c:forEach var="item" begin="1" end="10">
													<option value="${item}" <c:if test="${model.view.file_cnt == item }">selected="selected"</c:if> >${item}</option>
												</c:forEach>
                                        	</select>
                                        </li>
                                        <li>
                                        	<span class="list_t">설명</span>
                                        	<textarea name="description" id="editor">${model.view.description }</textarea>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                        ...
                        <div class="register_btn_area">
                            <div class="register_btn_con">
                                ...
                                <a class="storage" href="javascript:deleteClick()"> 삭제</a>
...
function insertClick()
{
	...
	var award_idx = '${model.view.idx}';
	
	$.ajax({
		url : '/admin/award/update.do',
		type : 'POST',
		data : formData,
		success: function(status , success) {
            ...
            if(cnt > 0){
                ...
                $(".require").each(function(index) {
                	...
                	var idx = $(this).find('input[name="require_idx"]').val();
                    ...
                    case 'update':
                    	...
                    	$.ajax({
	                    	url : '/admin/award/require/update.do',
	                		type : 'POST',
	                		data : ({
	                			idx : idx,
	                			award_idx : award_idx,
					...
					case 'delete':
						...
						$.ajax({
	                    	url : '/admin/award/require/delete.do',
	                		type : 'POST',
	                		data : ({
	                			idx : idx,
	                			award_idx : award_idx,
...
function deleteClick(){
	...
	var result = confirm('해당 참여 게시판을 삭제하시겠습니까?\n삭제한 참여게시판은 복구 불가능 합니다.')
...
$(document).ready(function () {
    ...
    $('#addRequirement').click(function () {
        var requirementHtml = '<li id="requirement_' + requirementIndex + '" class="require" data-type="insert">' +
                                  '<span class="list_t">필수 정보' + '</span>' +
...
//기존 필수 정보 항목 삭제 처리 함수 (requirementId 값을 직접 넘기지 않음)
function removeRequirement2(clickedButton) {
    var requirementElement = $(clickedButton).closest('li.require');

    // 항목을 숨기기
    requirementElement.hide();

    // data-type 속성을 'delete'로 설정
    requirementElement.attr('data-type', 'delete');
}
